...
/*** TO BE DONE 6.1 START ***/
	if(to_join[conn_no] == NULL) //per assicurarmi che il thread prec a conn_no non sia nullo
		return;

	for(i=MAX_CONNECTIONS; i<MAX_THREADS && to_join[conn_no] != &thread_ids[i]; i++); //trovo l'indice del conn_no che sto cercando

	pthread_mutex_lock(&threads_mutex);
	
	if(pthread_join(thread_ids[i], NULL) == -1) //attende terminazione thread
		fail_errno("Errore durante pthread_join");
...
/*** TO BE DONE 6.0 START ***/
	//race condition -> quando, avendo più thread, il risultato finale dipende dalla temporizzazione di esecuzione
	if(pthread_mutex_lock(&mime_mutex) != 0) //perchè non voglio che si possa accedere alle impostazioni seguenti 
...
/*** TO BE DONE 6.1 START ***/
	if(to_join[connection_idx] == NULL){ //se ho bisogno di assegnare valore il precedente
		to_join[connection_idx] = &thread_ids[new_thread_idx]; //precedente di curr è quello nuovo
		to_join[new_thread_idx] = NULL; //prec del nuovo punta a null
	}
	else{ //quello corrente ha già un precedente
