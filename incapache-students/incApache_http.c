...
/*** TO BE DONE 6.0 START ***/
	if (pthread_mutex_lock(&cookie_mutex) != 0)
		fail_errno("Errore durante lock");

	retval = CurUID++%MAX_COOKIES;
	UserTracker[retval] = 0; //array per tenere conto di utenti, infatti incrementando retval, andrò avanti nell'array

	if (pthread_mutex_unlock(&cookie_mutex) != 0)
		fail_errno("Errore durante unlock");
...
if (pthread_mutex_lock(&cookie_mutex) != 0)
		fail_errno("Errore durante lock");
	//usertracker tiene conto di quante volte accedo alle risorse
...
/*** TO BE DONE 6.0 START ***/
	stat_p = &stat_buffer; 
	if(stat(HTML_404, stat_p) != 0) //preparo stat_p a contenere le info HTML_404
...
/*** TO BE DONE 6.0 START ***/
	...
	snprintf(http_header + strlen(http_header), sizeof(http_header)-strlen(http_header), "%d", cookie); //scrivo nel buffer http_header il contenuto di cookie
...
/*** TO BE DONE 6.0 START ***/
	//se non da' errore ma invia una quantità inferiore di bytes?
	int check = sendfile(client_fd, fd, NULL, file_size);
	if(check == -1) //fatal error sendfile
		fail_errno("Errore durante sendifle");
		//controlla dimensione del file inviato, se è inferiore richiamo
	int c = 0; //counter per evitare loop
	while(check < file_size){ //se è andata a buon fine subito non entro nemmeno
		//offset aggriornato in aumatico dalla sendfile
		check += sendfile(client_fd, fd, NULL, file_size); //tengo in cosnsiderazione le sendfile precedenti
		//in modo da mantenere l'offset corretto e non rinviare stessi dati
		if(c >= 10){ //supero tot tentativi
...
#define MethodIsConditional(m) ((m)&METHOD_CONDITIONAL)
	FILE *client_stream = fdopen(client_fd, "r"); //in modalità lettura
...
/*** TO BE DONE 6.0 START ***/
		method_str = strtok_r(http_request_line, " ", &strtokr_save); //strtokr_save come pointer Per tenre il segno
...
/*** TO BE DONE 6.0 START ***/
					...
					if(option_val == NULL)
						perror("Option val got nullptr\n");
					UIDcookie = atoi(option_val); //converto da stringa a intero
...
/*** TO BE DONE 6.0 START ***/
					...
					if(strcmp(option_name, "If-Modified-Since") == 0){
						http_method |= METHOD_CONDITIONAL; //aggiungo il flag , se non è presente
