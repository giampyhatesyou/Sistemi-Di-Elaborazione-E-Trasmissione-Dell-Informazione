...
/*** TO BE DONE START ***/
	...
	if(sprintf(message, "%d", msg_no) < 0)
		fail_errno("Errore scrivendo nel buffer");
...
/*** TO BE DONE START ***/
	//funzione del .h che non richiede flag aggiuntivi
	sent_bytes = nonblocking_write_all(ping_socket, message, msg_size); //sent_bytes è il numero di byte invati
	if(sent_bytes < 0)
...
/*** TO BE DONE START ***/
	...
	/*while(re_try < timeout){
		if((recv_bytes = recv(ping_socket, answer_buffer, sizeof(answer_buffer), MSG_DONTWAIT)) > 0)
			break;
		++re_try;
	}
	recv_errno = errno;*/
	...
	//while non serve, viene fatto dopo
...
/*** TO BE DONE START ***/
	if(fcntl(ping_socket, F_SETFL, O_NONBLOCK) != 0){
		fail_errno("Socket cannot be changed to NONBLOCKING\n");
		return -1;
	}
...
if(getaddrinfo(argv[1], argv[2], &gai_hints, &server_addrinfo) != 0){ //indirizzo, numero di porta, flag e dove salvare
		perror("Err getaddrinfo");
		return -1;
	}
...
/*** TO BE DONE START ***/
	if ((ask_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, 0)) < 0){
...
/** TO BE DONE START ***/
	if(blocking_write_all(ask_socket, request, strlen(request)) != strlen(request)){
		fail_errno("Error while writing the request on socket\n");
	}
/*** TO BE DONE END ***/
