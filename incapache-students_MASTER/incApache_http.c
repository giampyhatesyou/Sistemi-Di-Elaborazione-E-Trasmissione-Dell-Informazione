...
/*** TO BE DONE 6.0 START ***/
	...
	while(check < file_size){ //se è andata a buon fine subito non entro nemmeno
		check = sendfile(client_fd, fd, NULL, file_size);
...
/*** TO BE DONE 6.0 START ***/
		method_str = strtok_r(http_request_line, " ", &strtokr_save); //strtokr_save come pointer oer tenre il segno
		filename = strtok_r(strtokr_save, " ", &strtokr_save);
		protocol = strtok_r(strtokr_save,"\r\n" , &strtokr_save);
