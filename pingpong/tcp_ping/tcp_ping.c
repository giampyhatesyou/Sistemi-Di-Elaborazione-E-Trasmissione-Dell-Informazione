...
/*** TO BE DONE START ***/
	...
	if(sprintf(message, "%d", msg_no) < 0)
		fail_errno("Errore scrivendo il msg_no sul buffer\n");
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_REALTIME, &send_time) != 0)
		fail_errno ("Error while setting the time in send_time");
...
/*** TO BE DONE START ***/
	sent_bytes = send(tcp_socket, message, msg_size, MSG_WAITALL);
	if(sent_bytes <= 0)
...
/*** TO BE DONE END ***/
	...
	for (offset = 0; (offset + (recv_bytes = recv(tcp_socket, rec_buffer + offset, sent_bytes - offset, MSG_WAITALL))) < msg_size; offset += recv_bytes) {
		...
		//printf("Offset: %zu, I received bytes: %zu \n", offset, recv_bytes);
		if (recv_bytes < 0)
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_REALTIME, &recv_time) != 0)
		fail_errno ("Error while storing the time\n");
...
/*** TO BE DONE START ***/
	gai_hints.ai_family = AF_INET;	//per ipv4
	gai_hints.ai_socktype = SOCK_STREAM;	//modalità stream
...
/*** TO BE DONE START ***/
	if(getaddrinfo(argv[1], argv[2], &gai_hints, &server_addrinfo) != 0) // ottengo info per connect arg[1] e 2
		fail_errno("Err getaddrinfo"); //salvo info in server_addrinfo
...
/*** TO BE DONE START ***/
	if ((tcp_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, 0)) < 0) //creo il nuovo tcp socket
		fail_errno("Error, failed to open socket");

	if(connect(tcp_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0) //connettendo tcp socket al server
		fail_errno("Error while connecting\n");
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	if(blocking_write_all(tcp_socket, request, strlen(request)) != strlen(request)) //funzione che ritorna i byte inviati
		return -1;
