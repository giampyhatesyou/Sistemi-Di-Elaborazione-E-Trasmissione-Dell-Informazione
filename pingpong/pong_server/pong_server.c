...
void tcp_pong(int message_no, size_t message_size, FILE *in_stream, int out_socket)
{
	...
	for (n_msg = 1; n_msg <= message_no; ++n_msg) {
		...
		for (cp = buffer, n_c = 0; n_c < message_size; ++n_c, ++cp) {
			...
			*cp = (char)cc;
		} //buffer è il mio messaggio, lo "riempio" usando cp come puntatore
...
/*** TO BE DONE START ***/
		...
		if((udp_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol)) < 0){
			fail_errno("Error craete udp socket: ");
		}
		...
		if(bind_rv = bind(udp_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) < 0){
			fail_errno ("Error, bind failed: ");
		}
		if(bind_rv ==0){	//se la bind() ha successo, ritorno il socket e salvo la porta
...
/*** TO BE DONE START ***/
		if(request_socket < 0)
			continue; //non ho richieste e skippo la funzione
		if((pid = fork()) == -1) //il padre continua ad ascolate per richieste
			fail_errno ("Error, fork failed: ");
		if(pid == 0) //il figlio gestisce le richieste
...
/*** TO BE DONE START ***/
	...
	if ((server_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol)) < 0){
		fail_errno("Error, failed open socket");
	}	//do' a server_socket l'indirizzo di server_addrinfo
	if (bind(server_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0){
		fail_errno ("Error, bind failed: ");
	}
	
	if (listen(server_socket, MAX_REQ) < 0){ //max req è il massimo di richieste che posso mettere in attesa
		fail_errno ("Error, listen failed: "); //se supero tot richieste errore
	}
