...
check_t check_cd(const line_t * const l)
{
	...
	//non necessario ma utile per specificare meglio l'errore
	for (int i = 1; i < l->n_commands; ++i) {
		if (l->commands[i]->n_args > 0 && strcmp(CD, l->commands[1]->args[0]) == 0) { //no strncmp perchè potrei avere una stringa che inizia con cd
			fprintf(stderr, "Error: cd must be the first command\n");
			return CHECK_FAILED;
		}
	}
...
void wait_for_children()
{
	...
	while((rip_child = wait(&exit_status)) > 0){
		if(WIFEXITED(exit_status) && WEXITSTATUS(exit_status)!=0) //se terminato normalmente, ma con exit-status!=0 
			fprintf(stderr, "Child PID is %d and was terminated with exit-status %d\n", rip_child, WEXITSTATUS(exit_status));
		else if(WIFSIGNALED(exit_status)) //se terminato da un segnale
			fprintf(stderr, "Child PID %d was terminated by signal %d (%s)\n", rip_child, WTERMSIG(exit_status), strsignal(WTERMSIG(exit_status)));
	}
	/*** TO BE DONE END ***/
}
...
void run_child(const command_t * const c, int c_stdin, int c_stdout)
{
	...
	if(f == 0){
		//Per debug
		//kill(getpid(), SIGINT);	//se impongo al figlio di "siucidarsi", posso verificare il segnale in arrivo al padre
...
void execute_line(const line_t * const l)
{
	...
	for(int a=0; a<l->n_commands; ++a) {
		...
		if (c->out_pathname) {
			...
			curr_stdout = open(c->out_pathname, O_CREAT | O_WRONLY, S_IWUSR | S_IRUSR);	//S_IWUSR=write permission S_IRUSR=read permission
			if(curr_stdout == -1){
